	...
	public function actionConfirmar() {
		...
		if (isset($_POST['FormInscricao'])) {
			...
			if($form->validate('inscricao')) {
				...
				$data = array(
					'condition'=>'mapa_vaga_publica = 0 and fconc_conc_id = :idconcurso and mapa_inst_id = :instid and fconc_func_id = :idfuncao',
					'join'=>'join funcao_concurso fc on mapa.mapa_fconc_id = fc.fconc_id_pk',
					'params'=>array('idconcurso' => $form->concurso->conc_id_pk, 'instid' => $form->instituicao->inst_id_pk, 'idfuncao' => $form->funcao->func_id_pk)
				 );
